...
class FFChatInfo {
  ...
  final List<PenggunaRecord> groupMembers;

  PenggunaRecord get currentUser => groupMembers
      .where((user) => user.reference == currentUserReference)
      .first;
  Map<String, PenggunaRecord> get otherUsers => Map.fromEntries(
        groupMembers
            .where((user) => user.reference != currentUserReference)
            .map((user) => MapEntry(user.reference.id, user)),
      );
  List<PenggunaRecord> get otherUsersList => otherUsers.values.toList();
...
class FFChatManager {
  ...
  Stream<FFChatInfo> getChatInfo({
    PenggunaRecord otherUserRecord,
    DocumentReference chatReference,
    ...
    return chatStream.asyncMap((chat) async {
      ...
      final groupMembers = await Future.wait(
        userRefs.map((ref) => PenggunaRecord.getDocument(ref).first),
      );
